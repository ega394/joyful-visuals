...
import KontakActions from "./components/KontakActions.jsx";
...
// ==================== WAKTU MULAI & SELESAI ====================
// Agenda menyimpan `jam` (wajib) dan `jamSelesai` (opsional, boleh kosong bila
// penyelenggara belum memberi informasi). Agenda lama tidak punya `jamSelesai`
// sama sekali, jadi semua helper di bawah harus aman terhadap nilai kosong.
const DURASI_DEFAULT=120; // menit — dipakai bila jam selesai tidak diketahui

// Jam selesai dianggap sah hanya bila lebih besar dari jam mulai (acara yang
// melewati tengah malam ditolak di validasi form, jadi tak perlu ditangani).
const jamSelesaiValid=ev=>!!(ev&&ev.jam&&ev.jamSelesai&&toMin(ev.jamSelesai)>toMin(ev.jam));

// "09:00 – 11:00" bila jam selesai diketahui, selain itu "09:00".
const fmtJam=ev=>{if(!ev)return "";const mulai=ev.jam||"";return jamSelesaiValid(ev)?mulai+" – "+ev.jamSelesai:mulai;};
// Versi dengan sufiks WITA: "09:00 – 11:00 WITA".
const fmtJamWita=ev=>{const t=fmtJam(ev);return t?t+" WITA":"";};
// Menit selesai untuk perhitungan — pakai jam selesai asli bila ada.
const menitSelesai=ev=>jamSelesaiValid(ev)?toMin(ev.jamSelesai):toMin(ev&&ev.jam)+DURASI_DEFAULT;
// Untuk kolom sempit (kalender, timeline, tabel): jam mulai di baris atas dan
// jam selesai di baris kecil di bawahnya, agar rentang tidak memaksa kolom
// melebar. Bila jam selesai kosong, tampilannya persis seperti sebelumnya.
const JamRingkas=({ev})=>jamSelesaiValid(ev)
...
function SambutanBlock({ev,canUpload,onUploadDocx,onUploadPdf,onRemove,onCommit,onDiscard}){
  ...
  // Naskah yang sudah diproses tapi BELUM dicatat ke agenda — menunggu
  // konfirmasi "Simpan Naskah" dari pengunggah.
  ...
  const handleFile=async(f)=>{
    ...
    if(isPdf){
      ...
      try{
        const res=await onUploadPdf(f,f.name);
        setPdfPreview(res?.pdfUrl||null);
        setPending({patch:res?.patch,nama:f.name});
    ...
    try{
      ...
      else if(result?.pdfUrl)setPdfPreview(result.pdfUrl);
      setPending({patch:result?.patch,nama:f.name});
      setStep("review");setProgress("");
    }catch(e){setStep("error");setErrMsg(e.message||"Gagal memproses file");}
  };

  // Konfirmasi: baru di sinilah naskah tercatat di agenda
  const simpanNaskah=async()=>{
    if(!pending?.patch)return;
    setSaving(true);
    try{
      await onCommit(pending.patch);
      setPending(null);setStep("done");
      globalToast("✅ Naskah sambutan tersimpan");
    }catch(e){setStep("error");setErrMsg(e.message||"Gagal menyimpan naskah");}
    finally{setSaving(false);}
  };

  const batalNaskah=async()=>{
    const p=pending;
    setPending(null);setPdfPreview(null);setStep("idle");
    if(ref.current)ref.current.value="";
    if(p?.patch&&onDiscard)onDiscard(p.patch);
  };
    ...
    return <div
      ...
      <div style={{textAlign:"center"}}>
        {isProcessing
          ...
          ?<>
            <div style={{fontSize:28,marginBottom:6}}>👀</div>
            <div style={{fontSize:13,fontWeight:800,color:"#4F46E5",marginBottom:3}}>Periksa Naskah Sebelum Disimpan</div>
            <div style={{fontSize:12,color:"#6B7280",marginBottom:10,wordBreak:"break-all"}}>{pending?.nama}</div>
            {pdfPreview&&<div style={{height:260,borderRadius:10,overflow:"hidden",border:"1.5px solid #C7D2FE",marginBottom:10,background:"#1a1a2e"}}>
              <iframe src={pdfPreview+"#toolbar=1&navpanes=0"} title="Pratinjau Naskah" style={{width:"100%",height:"100%",border:"none"}}/>
            </div>}
            <div style={{fontSize:11.5,color:"#92400E",background:"#FEF9C3",borderRadius:8,padding:"7px 10px",marginBottom:10,fontWeight:600}}>
              ⚠ Naskah belum tercatat. Tekan <b>Simpan Naskah</b> untuk mengonfirmasi.
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
              <button onClick={batalNaskah} disabled={saving}
                style={{padding:"10px 18px",borderRadius:9,border:"1.5px solid #CBD5E1",background:"white",color:"#475569",cursor:saving?"default":"pointer",fontSize:13,fontWeight:700}}>Batal</button>
              <button onClick={simpanNaskah} disabled={saving}
                style={{padding:"10px 24px",borderRadius:9,border:"none",background:saving?"#94A3B8":"linear-gradient(135deg,#059669,#047857)",color:"white",cursor:saving?"default":"pointer",fontSize:13,fontWeight:800,boxShadow:"0 3px 10px rgba(5,150,105,0.3)"}}>
...
function AIModal({onFill,onClose}){
  ...
  const analyze=async f=>{
    ...
    try{
      ...
      const PROMPT='Balas HANYA JSON ini tanpa penjelasan. Tanggal: YYYY-MM-DD, Jam: HH:MM. Isi "jamSelesai" HANYA bila undangan menyebut jam berakhir/selesai secara eksplisit, selain itu kosongkan: {"namaAcara":"","tanggal":"","jam":"","jamSelesai":"","penyelenggara":"","kontak":"","pakaian":"","jenisKegiatan":"","catatan":"","buktiUndangan":"","lokasi":"","untukPimpinan":["walikota"]}';
        ...
        {edited&&<>
          ...
          <div style={{display:"flex",gap:8,marginTop:16}}>
            ...
            <button onClick={async()=>{
              ...
              if(edited.jamSelesai&&toMin(edited.jamSelesai)<=toMin(edited.jam)){setValidErr("Jam selesai harus lebih besar dari jam mulai — kosongkan bila tidak diketahui.");return;}
              setValidErr("");
...
function SummaryModal({events,onToggleHide,onClose}){
  ...
  pub.forEach((ev,i)=>{
    ...
    shareLines.push(tglPrefix + fmtJamWita(ev) + lokasiSuffix);
    ...
    const rows=Object.keys(_byTgl).sort().flatMap(tgl=>{
      return _byTgl[tgl].map((ev,iG)=>{
        ...
        return (iG===0?"<tr class='ds'>":"<tr>")
          ...
          +"<td class='c'><b>"+(ev.jam||"")+"</b>"+(jamSelesaiValid(ev)?"<br><span class='sub'>s.d. "+ev.jamSelesai+"</span>":"")+"<br><span class='sub'>WITA</span></td>"
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 20px 20px"}}>
        ...
        <>{filtered.map(ev=><div key={ev.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,marginBottom:8,background:ev.tersembunyi?"#f8fafc":"white",border:"1px solid #f1f5f9",opacity:ev.tersembunyi?0.5:1}}>
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:13,color:"#64748b"}}>{mode==="range"?fmt(ev.tanggal)+" · ":""}{fmtJamWita(ev)} | {ev.penyelenggara}</div>
          </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 20px 20px"}}>
        ...
        <div style={{background:"#f8fafc",borderRadius:10,padding:"10px 14px",marginBottom:16,border:"1px solid #e2e8f0",maxHeight:200,overflowY:"auto"}}>
          ...
          filtered.map((ev,i)=><div key={ev.id} style={{display:"flex",gap:10,padding:"7px 0",borderBottom:i<filtered.length-1?"1px solid #f1f5f9":"none"}}>
            <div style={{width:44,flexShrink:0,textAlign:"center"}}><div style={{fontSize:12,fontWeight:700,color:NAVY}}>{getHari(ev.tanggal).slice(0,3)}</div><div style={{fontSize:12,fontWeight:800,color:"#334155"}}>{ev.tanggal.slice(8)}</div><div style={{fontSize:12,color:"#94a3b8"}}><JamRingkas ev={ev}/></div></div>
...
function DraftProgressView({events,user,upd,showT,askConfirm,setTab,isMobile,setForm,setEditId,deleteAndSync,onAddNew}){
  ...
  loadUsers().filter(u=>(u.role==="kasubbag_protokol")&&u.noWA).forEach(u=>sendWA({to:u.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,jamSelesai:ev.jamSelesai,penyelenggara:ev.penyelenggara,lokasi:ev.lokasi,event:"submit",submittedBy:user?.nama}));
  sendPush({targetRole:"kasubbag_protokol",title:"📋 Jadwal Baru Masuk",body:ev.namaAcara+" — "+fmtJamWita(ev),url:"/",tag:"submit-"+ev.id});
  sendPush({targetRole:"kasubbag_komdokpim",title:"📋 Jadwal Baru Masuk",body:ev.namaAcara+" — "+fmtJamWita(ev),url:"/",tag:"submit-"+ev.id});
            ...
            <div onClick={()=>setOpenId(open?null:ev.id)}
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:12,color:"#64748B",marginBottom:4}}>
                  {ev.tanggal} · {fmtJamWita(ev)} · {ev.penyelenggara||"—"}
                </div>
                {last&&(
        ...
        return <div key={ev.id} style={{background:"white",borderRadius:14,overflow:"hidden",
          ...
          <div onClick={()=>setExpandedMitra(expM?null:ev.id)} style={{padding:"14px 16px",cursor:"pointer",userSelect:"none"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              ...
              <div style={{fontSize:12,color:"#64748B",fontWeight:600}}>⏰ {fmtJamWita(ev)}</div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:3100,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"white",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:600,maxHeight:"94vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 -8px 40px rgba(0,0,0,0.25)"}}>
        ...
        <div style={{background:"linear-gradient(135deg,"+NAVY+",#1B4080)",padding:"18px 20px 14px",flexShrink:0}}>
          ...
          <div style={{color:"rgba(255,255,255,0.6)",fontSize:12,marginTop:3}}>{ev.tanggal} · {fmtJamWita(ev)}</div>
        </div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:3000,background:"rgba(0,0,0,0.55)",backdropFilter:"blur(4px)",display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"white",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:600,maxHeight:"92vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 -8px 40px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{background:"linear-gradient(135deg,"+NAVY+",#1B4080)",padding:"18px 20px 14px",flexShrink:0}}>
          ...
          <div style={{color:"white",fontSize:16,fontWeight:800,lineHeight:1.3}}>{ev.namaAcara}</div>
          <div style={{color:"rgba(255,255,255,0.6)",fontSize:12,marginTop:4}}>{ev.tanggal} · {fmtJamWita(ev)} · {ev.lokasi||"-"}</div>
        </div>
...
const fld=(k,l,type="text",full=false)=>(
  ...
  // Jam selesai opsional, tapi bila diisi harus lebih besar dari jam mulai.
  const jamSelesaiSalah = !!(form.jam && form.jamSelesai && toMin(form.jamSelesai)<=toMin(form.jam));
  const step1Valid = form.tanggal && form.jam && form.namaAcara && form.untukPimpinan.length>0 && !jamSelesaiSalah;
  ...
  return(
    <div style={{background:"white",borderRadius:12,padding:isMobile?"14px":"24px",
      ...
      {(wizStep===1||editId)&&<>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <input type="time" value={form.jam||""} onChange={e=>setForm(p=>({...p,jam:e.target.value}))}
                style={{flex:1,minWidth:0,padding:"10px 12px",borderRadius:9,border:"1.5px solid #e2e8f0",
                  color:"#1e293b",background:"white",fontSize:15,fontWeight:700,boxSizing:"border-box"}}/>
              <span style={{fontSize:14,color:"#94a3b8",fontWeight:700,flexShrink:0}}>s.d.</span>
              <input type="time" value={form.jamSelesai||""} onChange={e=>setForm(p=>({...p,jamSelesai:e.target.value}))}
                style={{flex:1,minWidth:0,padding:"10px 12px",borderRadius:9,
                  border:"1.5px solid "+(jamSelesaiSalah?"#FCA5A5":"#e2e8f0"),
                  color:"#1e293b",background:jamSelesaiSalah?"#FFF5F5":"white",fontSize:15,fontWeight:700,boxSizing:"border-box"}}/>
            </div>
            <div style={{fontSize:12,color:jamSelesaiSalah?"#DC2626":"#94a3b8",marginTop:4,fontWeight:jamSelesaiSalah?600:400}}>
              {jamSelesaiSalah?"⚠ Jam selesai harus lebih besar dari jam mulai (acara lintas hari belum didukung)":"Jam selesai opsional — isi bila informasinya sudah ada"}
            </div>
        ...
        {!editId&&<button type="button" onClick={()=>{if(!step1Valid){globalToast(jamSelesaiSalah?"Jam selesai harus lebih besar dari jam mulai":"Lengkapi dulu: Tanggal, Jam, Nama Acara, dan pilih Pimpinan","warn");return;}setWizStep(2);}}
          style={{width:"100%",padding:"14px",borderRadius:12,border:"none",
        ...
        {!editId&&<button type="button" onClick={()=>{if(!step1Valid){globalToast(jamSelesaiSalah?"Jam selesai harus lebih besar dari jam mulai":"Lengkapi dulu field wajib sebelum menyimpan","warn");return;}onSubmit();}}
...
// `targetUser` mengirim ke satu orang; `targetRole` ke seluruh pemegang peran.
// Pakai targetUser untuk pesan yang menyapa pribadi ("Anda Ditugaskan"),
// targetRole hanya untuk yang memang urusan bersama seperti antrian persetujuan.
async function sendPush({targetRole,targetUser,title,body,url,tag}){
  // Filter per kategori — skip jika flag-nya dimatikan
    ...
    <div onClick={()=>setExp(exp?null:ev.id)} style={{padding:"14px",cursor:"pointer",userSelect:"none"}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:11}}>
        ...
        <div style={{background:isToday?"linear-gradient(145deg,"+NAVY+",#1E3254)":"#F6F8FC",borderRadius:12,padding:"8px 7px",textAlign:"center",minWidth:46,flexShrink:0,boxShadow:isToday?"0 4px 12px rgba(10,22,40,0.25)":"none"}}>
          ...
          <div style={{fontSize:12,color:isToday?"rgba(255,255,255,0.6)":"#94A3B8",fontWeight:500,marginTop:1}}><JamRingkas ev={ev}/></div>
        </div>
        {/* Info */}
        <div style={{flex:1,minWidth:0}}>
    ...
    <tbody>
      {evList.map(ev=>{
        ...
        return <><tr key={ev.id} id={"ev-"+ev.id} className="card-row" style={{cursor:"pointer"}} onClick={()=>setExp(exp?null:ev.id)}>
          ...
          <td><span style={{fontWeight:700,fontSize:13,color:NAVY}}><JamRingkas ev={ev}/></span></td>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      <span style={{ fontSize: 13, color: "rgba(255,255,255,0.7)" }}>🕐 {fmtJamWita(ev)}</span>
...
async function generateAndShareAgendaCard(ev) {
  ...
  for(const row of rows){
    ...
    if(row.type==="time"){
      // Rentang jam ("09:00 – 11:00 WITA") lebih panjang — kecilkan sedikit
      // fontnya supaya tetap muat di kolom kartu.
      ctx.fillStyle="rgba(255,255,255,0.9)";setFont(ctx,jamSelesaiValid(ev)?26:30,"700");
      ctx.fillText((fmtJamWita(ev)||"-"),COL_X,cy);
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={()=>{
          ...
          askConfirm("Tolak Usulan Perubahan?","Usulan dibuang dan jadwal tetap tayang apa adanya. Admin RK dapat mengajukan usulan baru.",()=>{
            ...
            sendPush({targetUser:(ev.usulanEditOleh||ev.submittedBy),title:"❌ Usulan Perubahan Ditolak",body:ev.namaAcara+": "+catatan,url:"/",tag:"usulan-tolak-"+ev.id});
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={()=>{
          ...
          askConfirm("Tolak Usulan Perubahan?","Usulan dibuang dan jadwal tetap tayang apa adanya. Admin RK dapat mengajukan usulan baru.",()=>{
            ...
            sendPush({targetUser:(ev.usulanEditOleh||ev.submittedBy),title:"❌ Usulan Perubahan Ditolak Kabag",body:ev.namaAcara+": "+alasan,url:"/",tag:"usulan-tolak-"+ev.id});
...
function ExpandedDetail({ev,hariEv}){
  ...
  return <div>
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":needsSambutan(ev)?"1fr 1fr":"1fr",gap:"0 24px",marginBottom:14}}>
      <div>
        <div style={{display:"flex",flexDirection:"column",gap:5}}>
          {[{i:"Tgl",l:"Tanggal",v:hariEv+", "+fmt(ev.tanggal)},{i:"Jam",l:"Waktu",v:fmtJamWita(ev)},{i:"Org",l:"Penyelenggara",v:ev.penyelenggara},{i:"Tel",l:"Kontak",v:ev.kontak},{i:"No",l:"Bukti Undangan",v:ev.buktiUndangan},{i:"Bj",l:"Pakaian",v:ev.pakaian},{i:"Ket",l:"Catatan",v:ev.catatan}].filter(f=>f.v).map(f=>(
    ...
    {role==="admin_rk"&&<div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:8}}>
      ...
      {ev.alur==="draft"&&<>
        <button onClick={()=>{
          ...
          kasubbags.filter(u=>u.noWA).forEach(u=>sendWA({to:u.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,jamSelesai:ev.jamSelesai,penyelenggara:ev.penyelenggara,lokasi:ev.lokasi,event:"submit",submittedBy:user?.nama}));
          sendPush({targetRole:"kasubbag_protokol",title:"📋 Jadwal Baru Masuk",body:ev.namaAcara+" — "+fmtJamWita(ev),url:"/",tag:"submit-"+ev.id});
          sendPush({targetRole:"kasubbag_komdokpim",title:"📋 Jadwal Baru Masuk",body:ev.namaAcara+" — "+fmtJamWita(ev),url:"/",tag:"submit-"+ev.id});
        }}
    ...
    {role==="kabag"&&<div style={{display:"flex",flexDirection:"column",gap:7}}>
      {ev.alur==="menunggu_kabag"&&!ev.alurHapus&&<>
        ...
        <div style={{borderRadius:10,border:"1.5px solid #FECACA",overflow:"hidden"}}>
          ...
          <button onClick={()=>{if(!(rejectTexts[ev.id]||"").trim()){showT("Tulis alasan penolakan dulu","warn");return;}askConfirm("Tolak Jadwal?","Jadwal '"+ev.namaAcara+"' akan DITOLAK dan dikembalikan ke Admin RK dengan alasan di atas. Admin RK dapat memperbaiki dan mengajukan ulang.",()=>{upd(ev.id,{alur:"ditolak",catatanTolak:rejectTexts[ev.id]||"Perlu perbaikan",_requiresEdit:true});showT("Jadwal ditolak — dikembalikan ke Admin RK","warn");{const _ur=loadUsers().find(u=>u.username===ev.submittedBy);if(_ur?.noWA)sendWA({to:_ur.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,jamSelesai:ev.jamSelesai,penyelenggara:ev.penyelenggara,event:"rejected",catatanTolak:rejectTexts[ev.id]||"",submittedBy:getNamaByUsername(ev.submittedBy)});}sendPush({targetUser:ev.submittedBy,title:"❌ Jadwal Ditolak Kabag",body:ev.namaAcara+": "+(rejectTexts[ev.id]||"Perlu diperbaiki"),url:"/",tag:"rejected-"+ev.id});},"Ya, Tolak","#991B1B");}}
...
function TimelineView({evList}){
  ...
  const renderCard=(ev,i,arr)=>{
    ...
    // Bila jam selesai diketahui, "sedang berlangsung" dihitung dari rentang
    // sebenarnya. Agenda lama tanpa jam selesai tetap memakai aturan ±60 menit.
    const isNow=ev.tanggal===todayStr()&&(jamSelesaiValid(ev)
      ? nowMin>=evMin&&nowMin<=toMin(ev.jamSelesai)
      : Math.abs(evMin-nowMin)<60);
        ...
        <div onClick={()=>setExp(expandedId===ev.id?null:ev.id)} style={{
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{background:isNow?"linear-gradient(135deg,#0A1628,#1E3254)":isPast?"#F8FAFC":"#F0F4FF",
              ...
              <div style={{fontSize:13,fontWeight:900,color:isNow?"#C9A84C":isPast?"#94A3B8":"#0A1628",lineHeight:1}}><JamRingkas ev={ev}/></div>
  ...
  const arsipList=React.useMemo(()=>{
    ...
    events.filter(e=>e.tanggal&&e.tanggal.startsWith(bulanStr)).forEach(ev=>{
      if((filter==="semua"||filter==="undangan")&&ev.undanganFile&&ev.undanganNama){
        ...
        result.push({evId:ev.id,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,jamSelesai:ev.jamSelesai,
              ...
              {arsipList.map((item,i)=>(
                <div key={i} style={{background:"white",borderRadius:12,padding:"12px 14px",
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:12,color:"#64748B",marginTop:2}}>{fmtTgl(item.tanggal)} · {fmtJamWita(item)}</div>
...
export default function App(){
 ...
 const doBioLogin=async()=>{
  ...
  const handleSambutanDocx=useCallback(async(evId,file,ev)=>{
    ...
    // Tidak langsung disimpan — pengunggah memeriksa pratinjau dulu, lalu menekan
    // "Simpan Naskah" (commitSambutan) atau "Batal" (discardSambutan).
    const patch={sambutanFile:pdfUrl,sambutanNama:pdfName,sambutanDocx:docxUrl,sambutanDocxNama:docxName};
    return {pdfBase64,pdfUrl,patch};
  },[]);
  ...
  const handleSambutanUpload = useCallback(async (evId, file, name) => {
    ...
    if(SUPA_OK){
      ...
      if(url) return {pdfUrl:url,patch:{sambutanFile:url,sambutanNama:name}};
    }
    ...
    return {pdfUrl:b64,patch:{sambutanFile:b64,sambutanNama:name}};
  }, []);

  // Tahap 2: naskah baru tercatat di agenda setelah dikonfirmasi pengunggah
  const commitSambutan=useCallback((evId,patch)=>{
    updAndSync(evId,patch);
  },[updAndSync]);

  // Batal → bersihkan berkas yatim di storage (best-effort)
  const discardSambutan=useCallback(async(patch)=>{
    if(!patch)return;
    for(const url of [patch.sambutanFile,patch.sambutanDocx]){
      if(url&&!url.startsWith("data:")&&!url.startsWith("blob:")){
        await storageDelete("sambutan",url).catch(e=>console.warn("Sync:",e?.message||e));
      }
    }
  },[]);
  ...
  const saveEvaluasi=(evId, entry)=>{
    ...
    // Ringkasan kegiatan untuk isi notifikasi penugasan.
    const ringkasEv=(ev)=>(ev?.namaAcara||"Kegiatan")+" · "+(ev?.tanggal||"")+" "+fmtJamWita(ev);

    // Memberi tahu satu orang bahwa ia ditugaskan atau dicabut. Memakai
    // targetUser, bukan targetRole — sebelumnya seluruh pemegang peran ikut
    // menerima "Anda Ditugaskan" meski tidak ditugaskan sama sekali.
    const kabariPenugasan=async(un,ev,jenis,catatan)=>{
      const masuk=jenis==="masuk";
      await sendPush({
        targetUser:un,
        title: masuk?"📋 Anda Ditugaskan":"❌ Penugasan Dicabut",
        body:  ringkasEv(ev)+(masuk&&catatan?" · "+catatan:""),
        url:"/",
        tag:(masuk?"penugasan-":"cabut-")+ev?.id+"-"+un,
      });
    };
    ...
    // Hanya yang berubah yang dikabari. Yang sudah ditugaskan dan tidak
    // diapa-apakan tidak perlu menerima "Anda Ditugaskan" berulang kali
    // setiap atasannya menyunting daftar demi orang lain.
    for(const un of ditambah) await kabariPenugasan(un,ev,"masuk",catatanPenugasan);
    for(const un of dicabut)  await kabariPenugasan(un,ev,"cabut");
    setGlobalLoading(false);
  };

  // ── Cabut satu personil dari penugasan (aksi cepat dari PersonilBanner) ──
  const cabutPersonilSatu=async(ev,username,nama)=>{
    const newPersonil=(ev.personil||[]).filter(un=>un!==username);
    upd(ev.id,{personil:newPersonil});
    // Jalur ini dulu sama sekali tidak memberi tahu yang bersangkutan —
    // orangnya baru sadar saat kegiatannya hilang dari daftar tugasnya.
    await kabariPenugasan(username,ev,"cabut");
  ...
  React.useEffect(()=>{
    if(!user||!["timkom","kasubbag_komdokpim"].includes(role)){setNotifSambutan([]);return;}
    setNotifSambutan(events.filter(ev=>
      ev.alur==="disetujui"&&ev.sambutanPenyusun===user.username&&!ev.sambutanSah));
  },[events,role,user]);
...
const submit = async () => {
    ...
    // Jam selesai opsional; bila diisi wajib lebih besar dari jam mulai.
    if(form.jamSelesai&&toMin(form.jamSelesai)<=toMin(form.jam)){showT("Jam selesai harus lebih besar dari jam mulai.","error");setGlobalLoading(false);return;}
    ...
    if(editId!==null){
      ...
      if(evSebelum?.alur==="ditolak"){
        ...
        loadUsers().filter(u=>(u.role==="kasubbag_protokol")&&u.noWA).forEach(u=>sendWA({to:u.noWA,namaAcara:form.namaAcara,tanggal:form.tanggal,jam:form.jam,jamSelesai:form.jamSelesai,penyelenggara:form.penyelenggara,lokasi:form.lokasi,event:"submit",submittedBy:user?.nama}));
      ...
      <div style={{background:isMyTask?(isPast?"#F8FFFE":"#FAFFFD"):"#FAFBFF",borderRadius:14,border:"1.5px solid "+(isMyTask?(isPast?"#a7f3d0":"#6ee7b7"):"#E2E8F0"),marginBottom:8,overflow:"hidden",opacity:isPast&&!isMyTask?0.6:1}}>
        ...
        <div style={{padding:"12px 14px",display:"flex",gap:12,alignItems:"flex-start"}}>
          <div style={{minWidth:44,textAlign:"center",background:isMyTask?(isPast?"#d1fae5":"#ecfdf5"):"#f1f5f9",borderRadius:10,padding:"6px 0"}}>
            ...
            <div style={{fontSize:12,color:isMyTask?"#059669":"#94A3B8",fontWeight:600}}><JamRingkas ev={ev}/></div>
          </div>
          <div style={{flex:1,minWidth:0}}>
      ...
      <div style={{background:"white",borderRadius:16,marginBottom:10,overflow:"hidden",
        ...
        <div style={{padding:"12px 14px"}}>
          ...
          <div style={{fontSize:12,color:"#64748B",marginBottom:10}}>
            🕐 {fmtJamWita(ev)} &nbsp;·&nbsp; 📍 {ev.lokasi||ev.penyelenggara||"-"}
          </div>
          {/* WK */}
            ...
            <div key={ev.id} style={{background:"white",borderRadius:12,padding:"12px 14px",marginBottom:8,border:"1px solid #E2E8F0",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
              <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                <div style={{width:44,textAlign:"center",background:"#F0F4FF",borderRadius:10,padding:"6px 4px",flexShrink:0}}>
                  ...
                  <div style={{fontSize:12,color:"#94A3B8"}}><JamRingkas ev={ev}/></div>
                </div>
                <div style={{flex:1,minWidth:0}}>
...
function NotifCenter({events, user, onClose, isMobile}){
  ...
  const notifs=React.useMemo(()=>{
    ...
    // Penyusun naskah sambutan: tugas dari Kasubbag Komdokpim.
    // Turunan dari events — bertahan selama tugasnya belum tuntas.
    if(["timkom","kasubbag_komdokpim"].includes(role)){
      approved.filter(e=>e.sambutanPenyusun===user.username&&!e.sambutanSah).forEach(e=>{
        const belum=!e.sambutanFile;
        list.push({
          id:"sambutan-"+e.id,
          icon:"🎤",
          type:belum?"warn":"info",
          title:belum?"Tugas: Susun Naskah Sambutan":"Naskah Menunggu Pengesahan Kabag",
          body:e.namaAcara+" · "+fmt(e.tanggal)+" "+e.jam+" WITA"+
               (belum?" — naskah belum diunggah":" — sudah diunggah, menunggu Kabag"),
          ev:e,
        });
      });
    }
      ...
      <div style={{background:"white",borderRadius:14,marginBottom:10,overflow:"hidden",border:"1.5px solid #DBEAFE",boxShadow:"0 2px 10px rgba(10,22,40,0.07)"}}>
        <div style={{padding:"14px 16px",cursor:"pointer",display:"flex",gap:12,alignItems:"flex-start"}} onClick={()=>setExpanded(exp?null:ev.id)}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:13,color:"#64748B"}}>🕐 {fmtJam(ev)} · {ev.penyelenggara||ev.lokasi||"—"}</div>
    ...
    return(
      <div style={{background:"white",borderRadius:14,marginBottom:10,overflow:"hidden",border:"1.5px solid #D1FAE5",boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
        <div style={{padding:"13px 16px",cursor:"pointer",display:"flex",gap:12,alignItems:"flex-start"}} onClick={()=>setExpanded(exp?null:ev.id)}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:800,color:"#0F172A",marginBottom:3}}>{ev.namaAcara}</div>
            <div style={{fontSize:13,color:"#64748B"}}>🕐 {fmtJam(ev)} · {ev.penyelenggara||"—"}</div>
    ...
    <div style={{fontWeight:700,marginBottom:2}}>📝 Alasan Kabag menarik (perlu ditindaklanjuti):</div>{ev.catatanKabag}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div style={{borderRadius:10,overflow:"hidden",border:"1.5px solid #FECACA"}}>
              ...
              <button onClick={()=>askConfirm("Kembalikan untuk Diperbaiki?","Jadwal '"+ev.namaAcara+"' dikembalikan ke Admin RK dengan catatan di atas. Admin RK dapat mengedit dan mengajukan kembali — tidak perlu input dari awal.",()=>{upd(ev.id,{alur:"ditolak",catatanTolak:rejectTexts[ev.id]||"",_requiresEdit:true});showT("Dikembalikan ke Admin RK untuk diperbaiki","warn");const u=loadUsers().find(x=>x.username===ev.submittedBy);if(u?.noWA)sendWA({to:u.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,jamSelesai:ev.jamSelesai,penyelenggara:ev.penyelenggara,event:"rejected",catatanTolak:rejectTexts[ev.id]||"",submittedBy:getNamaByUsername(ev.submittedBy)});sendPush({targetUser:ev.submittedBy,title:"↩ Jadwal Perlu Diperbaiki",body:ev.namaAcara+": "+(rejectTexts[ev.id]||"Perlu perbaikan"),url:"/",tag:"rejected-"+ev.id});setExpanded(null);},"Ya, Kembalikan","#92400E")}
                ...
                <div style={{display:"flex",flexDirection:"column",gap:5}}>
                  {belumAda.slice(0,5).map(e=>(
                    <div key={e.id} style={{display:"flex",alignItems:"center",gap:8,background:"white",borderRadius:8,padding:"7px 10px",border:"1px solid #FECACA"}}>
                      ...
                      <span style={{fontSize:12,color:"#92400E",fontWeight:700,flexShrink:0}}>{fmtJamWita(e)}</span>
                    </div>
                  ))}
                ...
                {tugasan.length>0&&<div style={{borderTop:"1px solid #F1F5F9",padding:"8px 14px 12px"}}>
                  {tugasan.slice(0,3).map(ev=>(
                    <div key={ev.id} style={{display:"flex",gap:8,alignItems:"center",padding:"6px 8px",background:"#F8FAFF",borderRadius:8,marginBottom:4}}>
                      ...
                      <span style={{fontSize:12,color:"#64748B"}}><JamRingkas ev={ev}/></span>
                    </div>
                  ))}
      ...
      <div style={{background:"white",borderRadius:"18px 18px 0 0",padding:"20px 18px 32px",width:"100%",maxWidth:520,margin:"0 auto",boxSizing:"border-box"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>{
            ...
            // Notifikasi aplikasi menyusul WhatsApp. Sebelumnya jalur ini hanya
            // mengirim WA, sementara jalur cabut cepat tidak mengirim apa pun —
            // dua perilaku berbeda untuk tindakan yang sama.
            sendPush({targetUser:cabutTarget.un,title:"❌ Penugasan Dicabut",
                ...
                <div key={ev.id} style={{ padding:"11px 14px", borderBottom:"1px solid #F8FAFC", cursor:"pointer" }}
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8, marginBottom:6 }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:13, color:"#64748B" }}>{fmtDate(ev.tanggal)} · {fmtJamWita(ev)}</div>
                    </div>
          ...
          {hariIni.filter(ev=>ev.statusWWK).map(ev => (
            <div key={ev.id} style={{ padding:"10px 14px", borderBottom:"1px solid #F0FDF4", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                <div style={{ fontSize:13, color:"#64748B" }}>{fmtJamWita(ev)} · {ev.lokasi||"-"}</div>
              </div>
        ...
        <div onClick={()=>setExpanded(isExpanded?null:ev.id)} style={{padding:"16px",cursor:"pointer",userSelect:"none"}}>
          <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
            ...
            <div style={{minWidth:52,textAlign:"center",background:isToday?"linear-gradient(135deg,"+NAVY+",#1B4080)":"#F8FAFF",borderRadius:12,padding:"8px 4px",flexShrink:0}}>
              ...
              <div style={{fontSize:12,color:isToday?"rgba(255,255,255,0.7)":"#94A3B8",fontWeight:600}}><JamRingkas ev={ev}/></div>
            </div>
            <div style={{flex:1,minWidth:0}}>
...
// ==================== FORM ====================
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                          <span style={{fontSize:12,fontWeight:800,padding:"2px 8px",borderRadius:20,
                            ...
                            {isHariIni?"Hari ini":"Besok"} · {fmtJamWita(ev)}
                          </span>
                        </div>
  ...
  <div style={{minHeight:"100vh",width:"100%",background:NAVY,display:"flex"}}>
    ...
    {notifSambutan.filter(e=>!e.sambutanFile).length>0&&tab!=="form"&&notifPenugasan.length===0&&
      <div style={{position:"fixed",bottom:isMobile?70:20,left:"50%",transform:"translateX(-50%)",zIndex:2000,background:"#5B21B6",color:"white",borderRadius:12,padding:"10px 18px",fontSize:13,fontWeight:700,display:"flex",alignItems:"center",gap:10,boxShadow:"0 4px 20px rgba(0,0,0,0.3)",maxWidth:380,width:"calc(100% - 32px)",cursor:"pointer"}}
        onClick={()=>setShowNotifCenter(true)}>
        <span style={{fontSize:18}}>🎤</span>
        <div style={{minWidth:0}}>
          <div>{notifSambutan.filter(e=>!e.sambutanFile).length} naskah sambutan ditugaskan ke Anda</div>
          <div style={{fontSize:12,opacity:0.85,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
            {notifSambutan.filter(e=>!e.sambutanFile).map(e=>e.namaAcara).join(", ")}
          </div>
        </div>
        <button onClick={e=>{e.stopPropagation();setNotifSambutan([]);}} style={{marginLeft:"auto",background:"rgba(255,255,255,0.2)",border:"none",color:"white",borderRadius:6,padding:"2px 8px",cursor:"pointer",fontSize:12,flexShrink:0}}>✕</button>
      </div>
    }
