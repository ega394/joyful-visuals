...
// ── HTML #dokumen-cetak untuk Preview Iframe ──
...
export default function UndanganGenerator({ isMobile, showT }) {
  ...
  // ── buildIframeSrcDoc (Untuk Preview Kanan) ─────────────────────────────────────
  ...
  // ── generatePDF (Versi DOM Anti-Kosong) ────────────────────────────────
  const generatePDF = async () => {
    ...
    try {
      ...
      var htmlPdf =
        "<div id='dokumen-cetak' style='background: white;'>" +
        "<div class='halaman-a4'>" +
        "<div class='kop'>" +
        "<img src='/image001.jpg' alt='Garuda' style='width:88px; margin-bottom:5px;' onerror=\"this.style.display='none'\">" +
        "<div class='kop-teks'>WALI KOTA TARAKAN</div>" +
        "</div>" +
      ...
      // 1. Buat kontainer nyata di dalam body (bukan iframe) agar bisa dirender
      var container = document.createElement("div");
      // PERBAIKAN: Taruh di titik 0,0 agar tidak terpotong kamera html2canvas, tapi posisikan di belakang layar (z-index: -9999)
      container.style.cssText = "position:absolute; top:0; left:0; width:210mm; background:white; z-index:-9999; pointer-events:none;";
      ...
      contentWrapper.innerHTML = htmlPdf;
      container.appendChild(contentWrapper);

      document.body.appendChild(container);

      // 2. Jeda waktu krusial (1 detik) agar gambar lokal sempat dimuat sepenuhnya di memori browser
      ...
      // 3. Eksekusi PDF (scroll ke 0 agar posisi browser user tidak membuat PDF terpotong)
      await html2pdf().set({
        margin: 0, 
        filename: namaFile,
      ...
      // 4. Bersihkan dokumen bayangan setelah PDF selesai diunduh
