...
// ══════════════════════════════════════════════════════════════
// REKAP PENUGASAN BULANAN
// Akses: kabag (lihat semua tim), kasubbag_protokol, kasubbag_komdokpim
// ══════════════════════════════════════════════════════════════
...
// ── RankCard didefinisikan DI LUAR komponen utama ──
function RankCard({ s, rank, maxTugas }) {
  const [open, setOpen] = React.useState(false);

  const pct    = maxTugas > 0 ? s.jumlah / maxTugas : 0;
  const barClr = rank === 0 ? "#059669"
               : rank === 1 ? "#3B82F6"
               : rank === 2 ? "#F59E0B"
               : "#94A3B8";
        ...
        <div style={{ borderTop: "1px solid #F1F5F9", padding: "10px 14px 12px 58px", background: "#FAFBFF" }}>
          ...
          {s.kegiatan.map((ev, i) => (
            <div key={i} style={{ fontSize: 12, color: "#374151", marginBottom: 3, display: "flex", gap: 6 }}>
              ...
              <span>
                {ev.namaAcara}
                <span style={{ color: "#94A3B8" }}> — {ev.tanggal}</span>
              </span>
            </div>
          ))}
        </div>
...
// ── Komponen utama ──
export default function RekapPenugasanBulanan({ events, user, isMobile }) {
  const now = new Date();
  ...
  const allUsers   = React.useMemo(() => loadUsers(), []);
  const stafProto  = React.useMemo(() => allUsers.filter(u => ["staf", "admin_rk"].includes(u.role)), [allUsers]);
  const stafKomdok = React.useMemo(() => allUsers.filter(u => u.role === "timkom"), [allUsers]);
  ...
  const maxTugas    = activeRank.length > 0 ? activeRank[0].jumlah : 0;
  ...
  if (!isKabag && !isProto && !isKomdok) {
    return (
      <div style={{ padding: 40, textAlign: "center", color: "#94A3B8" }}>
        Fitur ini hanya tersedia untuk Kabag dan Kasubbag.
      </div>
    );
  }
    ...
    <div style={{ padding: isMobile ? "12px 14px" : "20px 28px", overflowY: "auto", flex: 1, background: "#F4F7FF" }}>
      ...
      <div style={{ background: "#F8FAFC", borderRadius: 14, padding: "14px 16px", border: "1px solid #E2E8F0" }}>
        ...
        {activeRank.length === 0 ? (
          ...
          activeRank.map((s, i) => (
            <RankCard key={s.username} s={s} rank={i} maxTugas={maxTugas} />
          ))
        )}
